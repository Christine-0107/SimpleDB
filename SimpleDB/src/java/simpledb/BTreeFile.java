 ...
 * @see simpledb.BTreeRootPtrPage#BTreeRootPtrPage //存指向root和第一个header的
 * @author Becca Taft
 */
public class BTreeFile implements DbFile {
	 ...
	 *            file. 对应的文件
	 * @param key - the field which index is keyed on 键
	 * @param td - the tuple descriptor of tuples in the file 这个表遵循的模式
	 */
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		//递归调用的出口是pid.pgcateg()是LEAF类型，返回这个叶节点表示的页
		if(pid.pgcateg()==BTreePageId.LEAF){
			//建议调用BTreeFile中的getPage方法取页
			//返回时的Permission需要使用传进来的perm
			return (BTreeLeafPage)this.getPage(tid,dirtypages,pid,perm);
		}
		else{ //如果还不是叶子，在内部节点中迭代，并比较值
			//获取的页应该是internal类型，Permission为只读
			BTreeInternalPage internalPage=(BTreeInternalPage)this.getPage(tid,dirtypages,pid,Permissions.READ_ONLY);
			//调用它的迭代器，迭代内部节点中的值和左右孩子的id(打包为一个BTreeEntry)
			Iterator<BTreeEntry> internalIterator=internalPage.iterator();
			BTreeEntry entry=null;
			while(internalIterator.hasNext()){
				entry=internalIterator.next();
				if(f==null){ //f要是为空，迭代返回最左侧
					//此时的id值通过entry调用getLeftChild()向左侧移动
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		/*1.在当前页的右侧添加一个新的页（叶节点），通过getEmptyPage()方法*/
		BTreeLeafPage newRightPage=(BTreeLeafPage)getEmptyPage(tid,dirtypages,BTreePageId.LEAF);
		/*2.移动一半的元素。先建立一个数组存放右侧元组；原节点删除右侧元素*/
		//反向迭代器
		Iterator<Tuple> reverseIterator= page.reverseIterator();
		//存放右侧元组的数组
		Tuple[] newTuples=new Tuple[(page.getNumTuples()+1)/2];
		int newTuplesNo=newTuples.length-1;
		while(newTuplesNo>=0&&reverseIterator.hasNext()){
			newTuples[newTuplesNo]= reverseIterator.next();
			newTuplesNo--;
		}
		//删除原节点右侧部分元组，放到新节点里
		for(int i=newTuples.length-1;i>=0;i--){
			page.deleteTuple(newTuples[i]);
			newRightPage.insertTuple(newTuples[i]);
		}
		/*3.获取中间值，复制给它的内部parent节点。通过方法getParentWithEmptySlots，RW权限*/
		Field midKey=newTuples[0].getField(keyField);
		BTreeInternalPage parent=getParentWithEmptySlots(tid,dirtypages, page.getParentId(),midKey);
		/*4.更新叶节点的左右指针*/
		BTreePageId oldRightPageId=page.getRightSiblingId(); //获取原来右侧节点的pageId
		newRightPage.setLeftSiblingId(page.getId()); //新右侧节点的左指针指向当前节点
		newRightPage.setRightSiblingId(oldRightPageId); //新右侧节点的右指针指向原右侧节点
		page.setRightSiblingId(newRightPage.getId()); //当前节点的右指针指向新的右侧节点
		if(oldRightPageId!=null){ //如果原来右侧不为空
			BTreeLeafPage oldRightPage=(BTreeLeafPage)getPage(tid,dirtypages,oldRightPageId,Permissions.READ_WRITE);
			oldRightPage.setLeftSiblingId(newRightPage.getId()); //原来右侧节点的左指针为新右侧节点
			dirtypages.put(oldRightPageId,oldRightPage); //原右侧页调整了，放到脏页列表里
		}
		/*5.更新双亲指针*/
		newRightPage.setParentId(parent.getId());
		page.setParentId(parent.getId());
		/*6.为新产生的要向上插入的Key设置entry，包括key值，左节点，右节点*/
		BTreeEntry newParentEntry=new BTreeEntry(midKey,page.getId(),newRightPage.getId());
		parent.insertEntry(newParentEntry); //向parent中插入这个新的entry
		/*7.添加脏页*/
		//原来右侧若不为空，已经添加过了
		dirtypages.put(page.getId(),page); //当前节点为脏页
		dirtypages.put(newRightPage.getId(),newRightPage); //新右侧节点为脏页
		dirtypages.put(parent.getId(),parent); //parent节点为脏页
		/*8.确定返回的是当前节点还是新的右侧节点，并返回*/
		if(midKey.compare(Op.GREATER_THAN,field)){ //右侧最左大于该值，返回当前页
			return page;
		}
		else{
			return newRightPage;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		/*1.在右侧添加一个新的页（内部节点），通过getEmptyPage()方法*/
		BTreeInternalPage newRightPage=(BTreeInternalPage)getEmptyPage(tid,dirtypages,BTreePageId.INTERNAL);
		/*2.移动一半的元素。先建立一个数组存放右侧entry；原节点删除右侧元素*/
		//反向迭代器
		Iterator<BTreeEntry> reverseIterator= page.reverseIterator();
		BTreeEntry[] newEntries=new BTreeEntry[(page.getNumEntries()+1)/2];
		int newEntriesNo=newEntries.length-1;
		while(newEntriesNo>=0&&reverseIterator.hasNext()){
			newEntries[newEntriesNo]= reverseIterator.next();
			newEntriesNo--;
		}
		//删除原节点右侧部分entry，放到新节点里
		BTreeEntry midEntry=null;
		for(int i=newEntries.length-1;i>=0;i--){
			if (i == 0) { //不添加了，只需要push到parent
				page.deleteKeyAndRightChild(newEntries[i]);
				midEntry = newEntries[0];
			}
			else {
				page.deleteKeyAndRightChild(newEntries[i]);
				newRightPage.insertEntry(newEntries[i]);
			}
		}
		/*3.获取中间entry，push给它的parent节点。通过方法getParentWithEmptySlots，RW权限*/
		midEntry.setLeftChild(page.getId());
		midEntry.setRightChild(newRightPage.getId());
		BTreeInternalPage parent;
		parent=getParentWithEmptySlots(tid,dirtypages, page.getParentId(),midEntry.getKey());
		parent.insertEntry(midEntry);
		/*4.更新双亲指针*/
		updateParentPointers(tid,dirtypages,newRightPage); //新的右侧成为了一部分节点的新parent
		page.setParentId(parent.getId());
		newRightPage.setParentId(parent.getId());
		/*5.添加脏页*/
		...
		/*6.确定返回的是当前节点还是新的右侧节点，并返回*/
		if(midEntry.getKey().compare(Op.GREATER_THAN,field)){ //右侧最左大于该值，返回当前页
			return page;
		}
		else{
			return newRightPage;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		BTreeLeafPage rightPage;//steal后右侧的页（通过它找到parent的entry）
		//将sibling比当前page多的个数的一半移过来
		int moveNum= (sibling.getNumTuples()-page.getNumTuples())/2;
		Tuple[] tuplesToMove=new Tuple[moveNum];
		int countNo=moveNum-1;
		Iterator<Tuple> it;
		if(isRightSibling){ //sibling为右侧
			rightPage=sibling;
			it= sibling.iterator();
		}
		else{ //sibling在左侧
		...
		while(countNo>=0&&it.hasNext()){
			tuplesToMove[countNo]= it.next();
			countNo--;
		}
		//sibling中删除，加到page里
		...
		//重新设置parent中指向左右孩子的entry的key，为右侧第一个key
	 ...
	 *
	 * 原来parent中的entry被拉下来到右侧第一个，左侧最后一个被拉上去到parent中做entry
	 *
	 * @param tid - the transaction id
	 * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
	 * @param page - the internal page which is less than half full
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry[] entriesToMove=new BTreeEntry[moveNum];
		int countNo=moveNum-1;
		Iterator<BTreeEntry> it= leftSibling.reverseIterator();
		while(countNo>=0&&it.hasNext()){
			entriesToMove[countNo]=it.next();
			countNo--;
		}
		//先将原来parent中entry的key添加到page中。
		BTreeEntry midEntry=new BTreeEntry(parentEntry.getKey(),
				leftSibling.reverseIterator().next().getRightChild(), /*左孩子为leftSibling最右entry的右孩子*/
				page.iterator().next().getLeftChild());/*右孩子为page最左entry的左孩子*/
		page.insertEntry(midEntry);
		//leftSibling中删除，除了最左侧点外加到page里
		for(int i=moveNum-1;i>0;i--){
			leftSibling.deleteKeyAndRightChild(entriesToMove[i]);
			page.insertEntry(entriesToMove[i]);
		}
		//最左侧点的key放到parent里
		leftSibling.deleteKeyAndRightChild(entriesToMove[0]);
		parentEntry.setKey(entriesToMove[0].getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		while(countNo<moveNum&&it.hasNext()){
			entriesToMove[countNo]=it.next();
			countNo++;
		}
		//先将原来parent中entry的key添加到page中。
		BTreeEntry midEntry=new BTreeEntry(parentEntry.getKey(),
				page.reverseIterator().next().getRightChild(), /*左孩子为page最右entry的右孩子*/
				rightSibling.iterator().next().getLeftChild());/*右孩子为rightSibling最左entry的左孩子*/
		page.insertEntry(midEntry);
		//rightSibling中删除，除了最右侧点外加到page里
		for(int i=0;i<moveNum-1;i++){
			rightSibling.deleteKeyAndLeftChild(entriesToMove[i]);
			page.insertEntry(entriesToMove[i]);
		}
		//最右侧点的key放到parent里
		rightSibling.deleteKeyAndLeftChild(entriesToMove[moveNum-1]);
		parentEntry.setKey(entriesToMove[moveNum-1].getKey());
		parent.updateEntry(parentEntry);
		//更新移动节点的child的parent
		updateParentPointers(tid,dirtypages,page);
		//设置脏页
	 ...
	 *
	 * 将右侧全部移到左侧，将parent中相应的key和右孩子指针删除
	 *
	 * @param tid - the transaction id
	 * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//将右侧节点的元组全部移动到左侧节点
		for(int i=0;i<moveNum;i++){
			rightPage.deleteKeyAndLeftChild(entriesToMove[i]);
			leftPage.insertEntry(entriesToMove[i]);
		}
